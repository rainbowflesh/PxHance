...
(() => {
    ...
    const CONFIG = {
        ...
        AUTORedirectToArtworks: DEFAULTS.AUTORedirectToArtworks
    };
    ...
    function registerMenu() {
        ...
        GM_registerMenuCommand("Set auto jump to all artworks page", () => {
            const val = prompt("Allow auto jump to all artworks page:", CONFIG.AUTORedirectToArtworks);
            if (val !== null) {
                GM_setValue("AUTORedirectToArtworks", Boolean(val));
                location.reload();
            }
        });
    ...
    function autoRedirectToArtworks() {
        ...
        const profileRegex = /^(https:\/\/www\.pixiv\.net\/(?:[a-zA-Z-]+\/)?users\/\d+)\/?$/;

        if (profileRegex.test(currentUrl)) {
            const newUrl = currentUrl.replace(profileRegex, '$1/artworks');
  ...
  `);
    ...
    function initObserver() {
        ...
        history.pushState = function () {
            ...
            // 路由变化时，检查并触发自动跳转
            setTimeout(autoRedirectToArtworks, 50);
        };
    ...
    if (document.readyState === "loading") {
        document.addEventListener(
            "DOMContentLoaded",
            () => {
                if (CONFIG.AUTORedirectToArtworks) {
